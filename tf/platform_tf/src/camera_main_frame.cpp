...
int main(int argc, char** argv){
 ...
 double x, y, yaw;
  ...
  while (nh.ok()){
    ros::NodeHandle nh;
    nh.getParam("tf_main_camera_x", x); 
    nh.getParam("tf_main_camera_y", y);
    nh.getParam("tf_main_camera_yaw", yaw);
    ...
    broadcaster.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "base_frame", "camera_main_frame"));
    rate.sleep();
  }
  return 0;
};